...
var RoleArns = {};
var LF = '\n';
var AWSGOVSTSEndpointDefaultRegion = 'us-gov-west-1';
...
// This adds an EventListener for each request to signin.aws.amazon.com (Global region) and signin.amazonaws.cn/saml (GOV region) 
...
// // This function runs on each request to https://signin.aws.amazon.com/saml (Global region) and https://signin.amazonaws-us-gov.com/saml (GOV region) 
...
function extractPrincipalPlusRoleAndAssumeRole(samlattribute, SAMLAssertion, SessionDuration) {
  ...
  // Pattern for Role (GOV region)
  var reRoleGOV = /arn:aws-us-gov:iam:[^:]*:[0-9]+:role\/[^,]+/i;  
  // Patern for Principal (SAML Provider) (GOV region)
  var rePrincipalGOV = /arn:aws-us-gov:iam:[^:]*:[0-9]+:saml-provider\/[^,]+/i;
  // Flag for region
  ...
  if (RoleArn == null) {
    RoleArn = samlattribute.match(reRoleGOV)[0];
    PrincipalArn = samlattribute.match(rePrincipalGOV)[0];
    GlobalRegion = false;
  } else {
    RoleArn = RoleArn[0];
  ...
  if (GlobalRegion) {
    ...
    var sts = new AWS.STS({region: AWSGOVSTSEndpointDefaultRegion});
  }
...
function assumeAdditionalRole(profileList, index, AccessKeyId, SecretAccessKey, SessionToken, docContent, SessionDuration, GlobalRegion) {
  ...
  if (GlobalRegion) {
    ...
    var options = {'accessKeyId': AccessKeyId, 'secretAccessKey': SecretAccessKey, 'sessionToken': SessionToken, region: AWSGOVSTSEndpointDefaultRegion};
  }
